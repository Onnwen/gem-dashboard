...
export default function CandidateResume({ candidateData }) {
  return (
    <div className={"flex flex-col justify-center items-center "}>
      ...
      <Badge color={candidateData.party_color_name} size="sm" className={"mt-1 text-sm"}>{candidateData.party_italian_name} • {(Number(candidateData.total_votes_percentage).toFixed(1))}%</Badge>
    </div>
  );
}